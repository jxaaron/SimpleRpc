...
public class SocketServerHandler extends SimpleChannelInboundHandler<RpcRequest> {
    private static final Logger logger = LoggerFactory.getLogger(SocketServerHandler.class);

    private final Map<String, Object> handlerMap;
    private final ThreadPoolExecutor serverHandlerPool;

    public SocketServerHandler(Map<String, Object> handlerMap, final ThreadPoolExecutor threadPoolExecutor) {
    ...
    public void channelRead0(final ChannelHandlerContext ctx, final RpcRequest request) {
        // filter beat ping
        if (Beta.BEAT_ID.equalsIgnoreCase(request.getRequestId())) {
    ...
    private Object handle(RpcRequest request) throws Throwable {
        ...
        String serviceKey = className+":"+version;
...
//        return serviceFastMethod.invoke(serviceBean, parameters);
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof IdleStateEvent) {
            ctx.channel().close();
            logger.warn("Channel idle in last {} seconds, close it", Beta.BEAT_TIMEOUT);
        } else {
